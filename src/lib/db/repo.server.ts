...
function mapearItem(f: FilaItem): Item {
  return {
    ...
    costo: num(f.cost),
    precio2: num(f.price2),
    precio3: num(f.price3),
    precio4: num(f.price4),
    precio5: num(f.price5),
    ...
    cantidad_empaque: num(f.pack_quantity),
    venta_minima: num(f.min_sale),
    venta_maxima: num(f.max_sale),
    existencia_maxima: num(f.max_stock),
    existencia_minima: num(f.min_stock),
    rotacion: num(f.rotation),
    alto: num(f.height),
    ancho: num(f.wide),
    profundidad: num(f.deep),
    medida_volumen_id: f.volume_measure_id ?? undefined,
    peso: num(f.weight),
    medida_peso_id: f.weight_measure_id ?? undefined,
    ficha: f.ficha ?? "",
    dias_antes_vencimiento: num(f.life),
    permitir_edicion_precio: Boolean(f.price_edition),
    arancel: num(f.arancel),
